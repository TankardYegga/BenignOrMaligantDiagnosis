# -*- encoding: utf-8 -*-
"""
@File    : densenet2.py.py
@Time    : 11/30/2021 10:36 AM
@Author  : Liuwen Zou
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(4, 4, 4, 4), num_init_features=64, bn_size=4, drop_rate=0,
                 num_classes=2, num_final_feats=10):
        ...
        #     OrderedDict([
        #         ('fc1', nn.Linear(num_features, num_final_feats)),
        #         ('fc2', nn.Linear(num_final_feats, num_classes))
        #     ])
        # )
...
if __name__ == '__main__':
    ...
    model = DenseNet(num_final_feats=10)
    outputs = model.features(inputs)
    print(outputs.shape)
    outputs = F.relu(outputs, inplace=True)
    outputs = F.avg_pool2d(outputs, kernel_size=7, stride=1).view(outputs.size(0), -1)
    outputs = model.classifier1(outputs)
    print(outputs.shape)
    outputs = model.classifier2(outputs)
    print(outputs.shape)
    print(outputs)
