...
from densenet_plus import Net2, Net4
...
def test_model(net, test_dataloader, img_datasets, use_gpu, img_feats_dict, save_path):
    ...
    test_loss = running_loss / len(img_datasets['test'])
    test_acc = running_corrects / len(img_datasets['test'])
    print('-' * 25, '*', '-' * 25)
    print("labels:", labels_list)
    print("preds:", preds_list)
...
@torch.no_grad()
def test_model2(net, test_dataloader, img_datasets, use_gpu, img_feats_dict, save_path, phase='test'):
...
if __name__ == '__main__':
    train_paths, train_labels = get_img_paths_and_labels(dir='/mnt/520/lijingyu/lijingyu/zlw/ExtraCode/data_corrected/shuffled_merged_data')
    ...
    #             '1_densenet121_w_trfm_fusion_2_cv_10_epoch_33_tensor(0.9286, device=\'cuda:0\')_best.pkl'
    # test_model2(model, dataloader['test'], img_datasets, use_gpu, img_feats_dict, save_path)
    # test_model2(model, dataloader['train'], img_datasets, use_gpu, img_feats_dict, save_path, phase='train')