...
"""
@File    : train14.py
@Time    : 12/31/2021 8:53 PM
@Author  : Liuwen Zou
@Email   : levinforward@163.com
@Software: PyCharm
交叉验证的纯densenet121模型 + 额外信息
"""
...
def train_model(i, net, train_paths, train_labels, val_paths, val_labels,
    ...
    val_consistent_mean, val_consistent_std = get_consistent_mean_and_std(val_paths, val_labels)
    data_transforms = {
...
def k_fold_train(k, train_img_path, train_img_label):
    ...
    end_epoch_number = k
    for i in range(start_epoch_number, end_epoch_number):
        ...
        optimizer = torch.optim.SGD( [{'params': weight_params, 'weight_decay': 1e-5},
         {'params': bias_params, 'weight_decay': 0}], lr=0.001, momentum=0.9)
        ...
        model_name = 'densenet121_2_new_cv_' + str(k)
    ...
    epoch_len = end_epoch_number - start_epoch_number
    print('train_loss:%.2f' % (train_loss_sum/epoch_len), 'train_acc:%.2f' % (train_acc_sum/epoch_len))
    print('val_loss:%.2f' % (val_loss_sum/epoch_len), 'val_acc:%.2f' % (val_acc_sum/epoch_len))
    print('*'*10, '最终交叉验证结果', '*'*10)
