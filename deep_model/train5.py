...
def train_model(model, dataloader, criterion, optimizer, scheduler,
    ...
    if os.path.exists(pkl_file):
        ...
        best_acc = (float)(os.path.basename(pkl_file).split('_')[-2])
        best_model_wts = model.state_dict()
    ...
    best_acc = best_acc.detach.numpy()
...
if __name__ == '__main__':
    ...
    #     OrderedDict([
    #         ('fc1', torch.nn.Linear(model_ft.fc.in_features, num_final_feats)),
    #         ('fc2', torch.nn.Linear(num_final_feats, 2))
    #     ]))
    #
    ...
    model = train_model(model_ft, image_dataloaders, criterion, optimizer, lr_scheduler, num_epochs,
                        use_gpu, image_datasets, model_name='coatnet',
                        pkl_file=r'')
