...
@Software: PyCharm
交叉验证的纯densenet121模型（只是改了fc的类别数目）
"""
...
def k_fold_train(k, train_img_path, train_img_label):
    ...
    for i in range(start_fold_th, k):
        ...
        model_name = 'densenet121_cv_' + str(k)
    ...
    print('train_loss:%.2f' % (train_loss_sum/(k - start_fold_th)), 'train_acc:%.2f' % (train_acc_sum/(k - start_fold_th)))
    print('val_loss:%.2f' % (val_loss_sum/(k - start_fold_th)), 'val_acc:%.2f' % (val_acc_sum/(k - start_fold_th)))
    print('*'*10, '最终交叉验证结果', '*'*10)
