...
densenet121 + extra_info + vgg + multi_fc + trfm_fusion
"""
...
def train_model(i, net, train_paths, train_labels, val_paths, val_labels,
    ...
    vgg_model.load_state_dict(torch.load(global_var.base_deep_prefix + 'pkls_vgg_19_pretrain_100/'\
    ...
    for epoch in range(num_epochs):
        ...
        for phase in running_stages:
            ...
            for j, (paths, inputs, labels) in enumerate(dataloader[phase]):
                ...
                # print('vgg_feats shape:', vgg_feats.shape)
                ...
                outputs = net.forward((inputs, traditional_feats, vgg_feats))
...
def k_fold_train(k, train_img_path, train_img_label):
    ...
    feats_csv_file = global_var.base_sklearn_prefix + '/feature_saved/merged_features/filtered_features.csv'
    ...
    start_fold_number = 0
    end_fold_number = k
    for i in range(start_fold_number, end_fold_number):
        ...
        model_name = 'densenet121_net7_cv_' + str(k)
    ...
    fold_length = end_fold_number - start_fold_number
    print('train_loss:%.2f' % (train_loss_sum/fold_length), 'train_acc:%.2f' % (train_acc_sum/fold_length))
    print('val_loss:%.2f' % (val_loss_sum/fold_length), 'val_acc:%.2f' % (val_acc_sum/fold_length))
    print('*'*10, '最终交叉验证结果', '*'*10)


if __name__ == '__main__':

    train_img_paths, train_img_labels = get_available_data_by_order(data_dir=global_var.base_data_prefix + '/shuffled_merged_data')
    k = 10
