...
# 在训练集上进行交叉验证的训练
# 然后再测试集上进行测试
def cv_train_model_on_part(model, param_grid, train_x, train_y, val_x, val_y,test_x, test_y, scoring='roc_auc', n_fold=10, model_name='unamed_model'):
    kfold = StratifiedKFold(n_splits=n_fold, shuffle=True, random_state=1)
    grid_search = GridSearchCV(
        model,
        param_grid,
    ...
    print('------------------------------validation------------------------------')
    val_y_predict = grid_search.predict_proba(val_x)[:, 1]
    val_y_predict_x = grid_search.predict(val_x)
    fpr, tpr, thresholds = roc_curve(val_y, val_y_predict)

    dict = {'labels': val_y, 'preds': val_y_predict}
    dict_filename = model_name + '_val_dicts.pickle'
    save_dir =  '/mnt/520/lijingyu/lijingyu/zlw/BenignOrMaligantDiagnosis/sklearn_model/extra_saved/saved_dicts/'
    ...
    print('y predict result:', val_y_predict)
    print('equal', np.all(val_y_predict_x == val_y))
    print("acc:", accuracy_score(val_y, val_y_predict_x, normalize=True))
    auc_score = roc_auc_score(val_y, val_y_predict)
    wrong_auc_score, sensitivity, specificity, ppv, npv = cal_metrics(labels_arr=val_y, preds_arr=val_y_predict_x)
    print('auc socre:', auc_score)
    print('sensitivity:', sensitivity)
    print('specificity:', specificity)

    print('------------------------------testing------------------------------')
    test_y_predict = grid_search.predict_proba(test_x)[:, 1]
    test_y_predict_x = grid_search.predict(test_x)

    dict = {'labels': test_y, 'preds': test_y_predict}
    dict_filename = model_name + '_test_dicts.pickle'
    save_dir =  '/mnt/520/lijingyu/lijingyu/zlw/BenignOrMaligantDiagnosis/sklearn_model/extra_saved/saved_dicts/'
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    ...
    print('equal', np.all(test_y==test_y_predict_x))
    print("acc:", accuracy_score(test_y, test_y_predict_x, normalize=True))
    auc_score = roc_auc_score(test_y,  test_y_predict)
    wrong_auc_score, sensitivity, specificity, ppv, npv = cal_metrics(labels_arr=test_y, preds_arr=test_y_predict_x)
    print('auc socre:', auc_score)
    print('sensitivity:', sensitivity)
    print('specificity:', specificity)
...
def get_fixed_split_data(feats_csv_file, datasets_str = 'fixed_split_3'):
    df = pd.read_csv(feats_csv_file)
    x = df.values[:, 3:]
    y = df.values[:, 2]
    imgs = df.values[:, 1]
    ...
    train_img_names = [os.path.basename(path).split('.')[0] for path in train_img_paths]
    val_img_paths, val_img_labels = get_available_data_by_order(data_dir=val_dir)
    val_img_names = [os.path.basename(path).split('.')[0] for path in val_img_paths]
    test_img_paths, test_img_labels = get_available_data_by_order(data_dir=test_dir)
    test_img_names = [os.path.basename(path).split('.')[0] for path in test_img_paths]
    ...
    for i in range(imgs.shape[0]):
        ...
        if img_name in train_img_names:
            train_list_x.append(x[i])
            train_list_y.append(y[i])
        elif img_name in val_img_names:
            val_list_x.append(x[i])
            val_list_y.append(y[i])
        elif img_name in test_img_names:
            test_list_x.append(x[i])
            test_list_y.append(y[i])
        else:
    ...
    test_list_x = np.asarray(test_list_x)
    test_list_y = np.asarray(test_list_y)
    # print(test_list_x.shape)
    # print(test_list_y.shape)

    train_list_x = np.asarray(train_list_x)
    train_list_y = np.asarray(train_list_y)
    # print(train_list_x.shape)
    # print(train_list_y.shape)

    val_list_x = np.asanyarray(val_list_x)
    val_list_y = np.asanyarray(val_list_y)
    # print(val_list_x.shape)
    # print(val_list_y.shape)

    train_label_encoder = LabelEncoder()
    train_label_encoder = train_label_encoder.fit(train_list_y)
    train_label_encoder_y = train_label_encoder.transform(train_list_y)

    test_label_encoder = LabelEncoder()
    test_label_encoder = test_label_encoder.fit(test_list_y)
    test_label_encoder_y = test_label_encoder.transform(test_list_y)

    val_label_encoder = LabelEncoder()
    val_label_encoder = val_label_encoder.fit(val_list_y)
    val_label_encoder_y = val_label_encoder.transform(val_list_y)

    return train_list_x, val_list_x, test_list_x, train_label_encoder_y, val_label_encoder_y, test_label_encoder_y
...
if __name__ == '__main__':
    ...
    data_x_train, data_x_val,  data_x_test, data_y_train,  data_y_val, data_y_test = get_fixed_split_data(feats_csv_file)
    ...
    # cv_train_model_on_part(knc_model, param_grid, data_x_train, data_y_train,  data_x_val, data_y_val, data_x_test, data_y_test, scoring, n_fold, 'knc')
    ...
    # cv_train_model_on_part(svc_model, param_grid, data_x_train, data_y_train,  data_x_val, data_y_val, data_x_test, data_y_test, scoring, n_fold, 'svc_on_whole_10')
    ...
    cv_train_model_on_part(rf_model, param_grid, data_x_train, data_y_train, data_x_val, data_y_val, data_x_test, data_y_test,
